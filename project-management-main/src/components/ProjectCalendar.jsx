...
const ProjectCalendar = ({ tasks }) => {
    ...
    const upcomingTasks = tasks
        .filter((task) => task.due_date && !isNaN(new Date(task.due_date)) && !isBefore(new Date(task.due_date), today) && task.status !== "DONE")
        .sort((a, b) => new Date(a.due_date) - new Date(b.due_date))
        .slice(0, 5);

    const overdueTasks = tasks.filter((task) => task.due_date && !isNaN(new Date(task.due_date)) && isBefore(new Date(task.due_date), today) && task.status !== "DONE");
